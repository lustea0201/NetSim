 ...
 */
    ...
    /*! 
  Tests whether there is a link between two nodes 
  @param[in] n,m the indexes if the two nodes 
  @param[out] true if the link exists between the two nodes (in any direction), false otherwise.
 */
    bool is_link(const size_t&, const size_t&);
